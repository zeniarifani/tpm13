...
function validateConfirmPassword(){
  ...
  if (valuePassword == ""){
    errorConfirmPassword.innerHTML = "Password cannot be empty"
    inputPassword.style.border = "2px solid red"
    return false;
  } else if (!valueConfirmPassword === valuePassword){
    errorConfirmPassword.innerHTML = "Confirm Pass must be the same with password"
    inputPassword.style.border = "2px solid red"
    return false;
  } else if (valuePassword.length < 8){
    errorConfirmPassword.innerHTML = "Password must be longer than 8!"
    inputPassword.style.border = "2px solid red"
    return false;
  } else if (!/[A-Z]/.test(valuePassword)){
    errorConfirmPassword.innerHTML = "Password must contain at least 1 uppercase letter!"
    inputPassword.style.border = "2px solid red"
    return false;
  } else if (!/[a-z]/.test(valuePassword)){
    errorConfirmPassword.innerHTML = "Password must contain at least 1 lowercase letter!"
    inputPassword.style.border = "2px solid red"
    return false;
  }else if (!/\d/.test(valuePassword)){
    eerrorConfirmPassword.innerHTML = "Password must contain at least 1 digit"
    inputPassword.style.border = "2px solid red"
    return false;
  } else if (!/[!@#$%^&*(),.?":{}|<>]/.test(valuePassword)){
    errorConfirmPassword.innerHTML = "Password must contain at least 1 special character"
    inputPassword.style.border = "2px solid red"
    return false;
  } else {
...
nextButton.addEventListener("click", function(e){
  ...
  if (isNameTrue && isPassTrue && isStatusTrue && inputConfirmPassword) {
    ...
    form.innerHTML = `
      <input type="hidden" name="name" value="${document.getElementById("nama-group").value}">
      <input type="hidden" name="password" value="${document.getElementById("password").value}">
      <input type="hidden" name="binusian" value="${document.querySelector('input[name="binusian"]:checked').value}">
    `;
